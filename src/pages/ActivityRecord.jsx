...
const ActivityRecord = () => {
  ...
  // 기본 카테고리 메뉴 스타일
  const categoryBarMenuClass = 'flex-1 content-center bg-navy-lightSide cursor-pointer hover:bg-gray-lightSide hover:rounded-[10px]';
  // 현재 위치한 카테고리 메뉴 스타일
  ...
  return (
    <div className="activity-container flex-col">
      <div className="category-bar bg-navy-lightSide flex w-[1492px] h-[61px] mx-[70px] mt-[84px] rounded-[10px]">
        ...
        <div onClick={() => nav('/user/growth/activity/contest')} className={`${isActive("/user/growth/activity/contest")}`}>공모전</div>
        <div onClick={() => nav('/user/growth/activity/club')} className={`${isActive("/user/growth/activity/club")}`}>동아리</div>
        <div onClick={() => nav('/user/growth/activity/extra')} className={`${isActive("/user/growth/activity/extra")} rounded-r-[10px]`}>여분통</div>
      </div>
