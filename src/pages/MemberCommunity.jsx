...
import PostMemberItem from '../components/PostMemberItem';
	...
	const PostData = [
		{
			...
			community_post_id: 1,
			user_nickname: '용가리',
			community_post_createdTime: '5분전',
			user_position: '프론트엔드',
			postTitle: '면접장에서 질문이',
			content:
		...
		},
		{
			user_profile: 프사,
			community_post_id: 2,
			user_nickname: 'greatSound',
			community_post_createdTime: '10분전',
			user_position: '디자이너',
			postTitle: '면접장에서 질문이',
			content:
				'나이가 많으시네요 이거 하나만 물어보고 저한테는 더이상 질문이 없으면 거의\n탈락이라고 생각해야 하나요 다름분들에게는 이것저것 질문이 있던데 저는 저게 다 입니다.\n탈락이라고 생각해야 할까요 나미 40대인 게 죄인가요',
						...
						<div className="mt-[51px] space-y-[51px]">
							{PostData.map((data, index) => (
								<PostMemberItem
									...
									id={data.community_post_id}
									createdTime={data.community_post_createdTime}
